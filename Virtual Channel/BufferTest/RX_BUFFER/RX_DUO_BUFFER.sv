...
RX_FIFO #(.DATA_WIDTH(DATA_WIDTH)) Header (.clk(clk), .rst(rst), .WrEn(HWrEn), .RdEn(HRdEn), .DataIn(IN_TLP_DW), .DataOut(HEADER_OUT_TLP_DW), .comb_DataOut(HEADER_OUT_TLP_DW_COMB), .FullPending(HFull), .Empty(HEmpty),
.flush(flush), .commit(commit));

RX_FIFO #(.DATA_WIDTH(DATA_WIDTH)) Data   (.clk(clk), .rst(rst), .WrEn(DWrEn), .RdEn(DRdEn), .DataIn(IN_TLP_DW), .DataOut(DATA_OUT_TLP_DW), .comb_DataOut(DATA_OUT_TLP_DW_COMB), .FullPending(DFull), .Empty(DEmpty),
.flush(flush), .commit(commit));
